...
interface FeatureCardProps {
  icon: string; title: string; description: string; variant?: 'primary' | 'secondary' | 'accent';
}

export function FeatureCard({ icon, title, description, variant = 'primary' }: FeatureCardProps) {
  const backgroundColors = { primary: '#ffffff', secondary: '#f0f8ff', accent: '#fff8f0' };
  return (
...
export function ResponsiveImage({ source }: { source: { uri: string }, style?: object }) {
  const { width } = useWindowDimensions();
  return (
    <View style={[styles.imageContainer, { width: width > 768 ? '100%' : width - 32 }]}>
      <View style={[styles.imagePlaceholder, { aspectRatio: 16 / 9 }]}>
        <Text style={styles.imagePlaceholderText}>Image {source.uri}</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: '#0066cc', paddingVertical: 12, paddingHorizontal: 16, paddingTop: Platform.OS === 'ios' ? 44 : 12 },
  main: { flex: 1 },
  ...
  footer: { backgroundColor: '#ffffff', paddingVertical: 16, paddingHorizontal: 16, borderTopWidth: 1, borderTopColor: '#e0e0e0' },
  featureCard: { borderRadius: 16, padding: 20, marginBottom: 16, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  ...
  statsRow: { flexDirection: 'row', justifyContent: 'space-around', backgroundColor: '#ffffff', paddingVertical: 20, paddingHorizontal: 16, borderRadius: 12, marginBottom: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 2, elevation: 2 },
  ...
  imageContainer: { marginBottom: 16, alignSelf: 'center' },
  imagePlaceholder: { backgroundColor: '#e0e0e0', alignItems: 'center', justifyContent: 'center', borderRadius: 8, overflow: 'hidden' },
  imagePlaceholderText: { fontSize: 16, color: '#999' }
});